...
function mod:handleCollectiblesCap(pick, colllider)
    ...
    if player then
        ...
        if pick.Variant == 20 then
            ...
            if player:GetName() == "Keeper" or player:HasCollectible(CollectibleType.COLLECTIBLE_STAIRWAY) or player:HasCollectible(CollectibleType.COLLECTIBLE_POUND_OF_FLESH) then
                cap = cap + 10
            end
...
mod:AddCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, mod.handleCollectiblesCap)